        ...
        <div className="flex items-center gap-4 lg:gap-10">
          ...
          <div ref={searchRef} className="hidden md:block relative">
            <form onSubmit={handleSearch} className="flex items-center group bg-white/5 rounded-full px-5 py-2.5 border border-white/5 focus-within:border-primary/30 transition-all">
              <svg className={`w-4 h-4 transition-colors ${isLoading ? 'text-primary animate-pulse' : 'text-gray-500 group-focus-within:text-primary'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {showDropdown && (query.trim() !== "") && (
              <div className="absolute top-14 right-0 w-80 bg-[#121822] rounded-[1.5rem] shadow-2xl border border-white/5 overflow-y-auto max-h-[400px] z-[1001] p-1 scrollbar-thin">
                {isLoading ? (
                  <div className="p-8 text-center text-gray-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Searching...</div>
                ) : results.length > 0 ? (
                  results.map((movie) => (
                    <div 
                      key={movie.id} 
                      onClick={() => { navigate(`/movie/${movie.id}`); setShowDropdown(false); setQuery(""); }} 
                      className="flex items-center gap-4 p-4 hover:bg-white/5 cursor-pointer transition rounded-[1.2rem] border-b border-white/[0.03] last:border-0"
                    >
                      <img 
                  ...
                  <div className="p-8 text-center text-gray-500 text-[10px] font-black uppercase tracking-widest">No results found</div>
                )}
              </div>
            )}
          </div>
          ...
          {user ? (
            <div ref={profileRef} className="relative flex items-center gap-3">
              <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white/80 hidden sm:block">
                {user.name}
              </span>
              <button 
                onClick={() => setShowProfileMenu(!showProfileMenu)} 
                className="w-9 h-9 rounded-full border-2 border-primary/20 p-0.5 hover:border-primary transition-all overflow-hidden focus:outline-none"
              >
              ...
              {showProfileMenu && (
                <div className="absolute top-12 right-0 w-44 bg-[#121822] rounded-[1.2rem] border border-white/10 shadow-2xl p-2 z-[1001]">
                  <div className="px-4 py-2 border-b border-white/5 mb-1 sm:hidden">
                    <p className="text-[9px] font-black uppercase text-white/40 tracking-widest">{user.name}</p>
                  </div>
                  <button 
                    onClick={logout} 
            ...
            <Link to="/login" className="hidden sm:block">
              <button className="px-6 py-2 bg-primary text-black rounded-full text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg shadow-primary/20">
                Login
              </button>
            </Link>
          )}
        ...
        <div className="lg:hidden absolute top-full left-0 right-0 bg-[#0b0d14] border-b border-white/5 p-6 flex flex-col gap-8 shadow-2xl animate-fade-in-down">
          ...
          <form onSubmit={handleSearch} className="flex md:hidden items-center bg-white/5 rounded-full px-5 py-4 border border-white/5">
            ...
            <input 
              ...
              value={query} 
              onChange={(e) => setQuery(e.target.value)} 
              className="bg-transparent border-none outline-none text-white text-[10px] font-black uppercase tracking-widest ml-4 w-full" 
            />
          </form>
          ...
          <div className="flex flex-col gap-4">
            {mainLinks.map((link) => (
              ...
              >
                {link.name}
                {link.showBadge && watchlist.length > 0 && (
                  <span className="bg-primary text-black px-2 py-0.5 rounded-full text-[10px] leading-none">
                    {watchlist.length}
                  </span>
                )}
              </Link>
            ...
            {!user && (
              <Link to="/login" onClick={() => setIsMenuOpen(false)} className="mt-4 bg-primary text-black text-center py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] shadow-lg">
                Login Access
              </Link>
            )}
          </div>
